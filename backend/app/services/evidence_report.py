...
def build_blackglass_report(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    finding_rows = [
        [
            ...
            localized(", ".join(str(citation["evidence_id"]) for citation in item["citations"])),
        ]
    ...
    for index, (kind, en, dv) in enumerate(SECTIONS, start=1):
        ...
        elif kind in {"osp-key-findings", "osp-integrated-profile"}:
            body = "Evidence-backed findings generated by cyber-ai." if findings else unsupported
            rows = finding_rows
