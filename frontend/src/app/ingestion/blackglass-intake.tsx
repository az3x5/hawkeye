...
type IngestResult = {
  asset: Asset;
  source: { source_uuid: string; source_system: string; external_source_id?: string | null };
  created: boolean;
  status: "stored" | "already_held";
};
...
export function BlackGlassIntake({ canRead }: { canRead: boolean }) {
  ...
  async function submit() {
    ...
    form.set("external_source_id", externalId.trim());
  ...
  return <div className="space-y-5">
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(19rem,0.8fr)]">
      ...
      <aside className="space-y-4">
        ...
        {result ? <div className="panel border-accept/25 p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><CheckCircle2 className="size-4 text-accept" /><h2 className="text-sm font-semibold text-ink">Delivery accepted</h2></div><StatusBadge tone="accept">{result.status === "stored" ? "stored" : "already held"}</StatusBadge></div><dl className="mt-4 grid gap-3 text-xs"><div><dt className="text-ink-faint">EagleEye asset</dt><dd className="mt-1 break-all font-mono text-ink">{result.asset.media_uuid}</dd></div><div><dt className="text-ink-faint">Source record</dt><dd className="mt-1 break-all font-mono text-ink">{result.source.external_source_id}</dd></div><div><dt className="text-ink-faint">SHA-256</dt><dd className="mt-1 break-all font-mono text-ink-muted">{result.asset.sha256}</dd></div></dl></div> : null}
      </aside>
    </div>
